...
import { Upload, FileCode, Settings, TrendingUp, Copy, Star } from 'lucide-react'
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-400">Upload and manage your trading robots</p>
        </div>
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-2xl p-8 mb-8">
          ...
          <div className="space-y-6">
            ...
            {selectedFile && (
              ...
              >
                {uploading ? 'Uploading & Analyzing...' : 'Upload & Extract Parameters'}
              </button>
            )}
          </div>
        </div>
        ...
        {uploadResult && (
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-2xl p-8 mb-8">
            ...
            <div className="bg-slate-900/50 rounded-xl p-6 mb-6">
              <h3 className="text-lg font-semibold mb-4 text-blue-400">EA Information</h3>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-semibold">{(uploadResult.ea.file_size / 1024).toFixed(2)} KB</p>
                </div>
                <div>
                  <p className="text-slate-400 text-sm">Source Code</p>
                  <p className="font-semibold">{uploadResult.ea.has_source ? '✓ Available' : '✗ Not Available'}</p>
                </div>
              </div>
            </div>
            ...
            {uploadResult.parameters.extracted > 0 && (
              <div className="bg-slate-900/50 rounded-xl p-6">
                <h3 className="text-lg font-semibold mb-4 text-purple-400">Extracted Parameters</h3>

                <div className="grid grid-cols-3 gap-4 mb-6">
                  <div className="bg-slate-800 rounded-lg p-4 text-center">
                    <p className="text-3xl font-bold text-blue-400">{uploadResult.parameters.extracted}</p>
                    <p className="text-slate-400 text-sm">Total Parameters</p>
                  </div>
                  <div className="bg-slate-800 rounded-lg p-4 text-center">
                    <p className="text-3xl font-bold text-green-400">{uploadResult.parameters.summary.optimizable_count}</p>
                    <p className="text-slate-400 text-sm">Optimizable</p>
                  </div>
                  <div className="bg-slate-800 rounded-lg p-4 text-center">
                    <p className="text-3xl font-bold text-purple-400">{uploadResult.parameters.summary.group_count}</p>
                ...
                <div className="space-y-2 mb-6">
                  <p className="text-sm font-semibold text-slate-400 mb-2">Parameter Groups:</p>
                  <div className="flex flex-wrap gap-2">
                    {uploadResult.parameters.summary.groups?.map((group: string, idx: number) => (
                      <span key={idx} className="bg-slate-800 px-3 py-1 rounded-full text-sm">
                        {group}
                      </span>
                    ))}
                  </div>
                ...
                <div className="space-y-2">
                  <p className="text-sm font-semibold text-slate-400 mb-2">Sample Parameters:</p>
                  <div className="space-y-2">
                    {uploadResult.parameters.details?.map((param: any, idx: number) => (
                      <div key={idx} className="bg-slate-800 rounded-lg p-3">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="font-mono font-semibold">{param.name}</p>
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-sm mt-1 text-slate-300">= {param.default_value}</p>
                          </div>
                        </div>
                      </div>
                    ))}
